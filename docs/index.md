...
| `ontology-sqlite-index` | [Ontology SQLite Index](codebase.md#ontology-harmonizer) | Shared OntoStore SQLite lookup index, lazy refresh, framework manipulation helpers, and cache-builder synchronization. | SQLite, sqlite_path, ontologies.sqlite3, index_framework, remove_indexed_framework, sync_sqlite, freshness, WAL, lookup |
| `ontology-search-judge` | [Ontology Search Judge](codebase.md#ontology-harmonizer) | Two-stage LLM validation and compact per-call context contracts for OLS candidates and grounded web evidence. | search_llm_judge, judge_search_results, judge_search.md, LLM context, semantic target, top 10, complete description, restricted OLS, unrestricted OLS, Gemini grounded search, fail closed, search_llm_judgements |
