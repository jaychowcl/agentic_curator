...
| --- | --- | --- | --- |
| `project-purpose-and-layout` | [Project Purpose And Layout](codebase.md#project-purpose-and-layout) | Repository purpose, tracked layout, README interface/configuration guide, canonical authors-header policy, citation, and ignored/external files. | layout, purpose, README, configuration, quickstart, Python API, CLI guide, Docker unsupported, authors, author header, exactly one, Markdown prompt comment, linked affiliations, Please cite us using, software citation, LICENSE, external source, ignored, .dev, .env, .vscode |
