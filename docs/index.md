...
| `ontology-harmonizer` | [Ontology Harmonizer](codebase.md#ontology-harmonizer) | Ontology harmonizer curator, public harmonize method, compact metadata context, per-target LLM call counts, OntoStore lookup/downloads, field harmonization, strategies, and target extraction/application. | OntologyHarmonizer, metadata_context, 500 characters, publication_context, LLM calls per target, lookup_label, assign_field, assign_onto_framework, harmonize_label, miniml_json, OntoStore, FTS5, OLS4, websearch, target paths |
